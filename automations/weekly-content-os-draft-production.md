...
2. Selected eligible Research Bank items using the eligibility and cooldown rules in this contract.
3. Created exactly:
   - 1 Content Library `Newsletter Issue` draft
   - 3 Content Library `Linkedin Post` drafts
4. Updated the selected Research Bank items:
   - `Status=Used`
   - `Selected Week` set to the run date (YYYY-MM-DD)
5. Created exactly 1 Inbox review task linking all 4 draft assets.
6. Reported created/updated URLs, changed fields, and skipped candidates with reasons.
...
- Inbox task data source (review task): `collection://24cec550-1950-47d4-b00f-16e0809faa54`
...
2. Fetch Notion targets before writing:
...
3. Build a candidate list (search+fetch fallback only):
...
4. Select the weekly batch deterministically:
...
5. Draft content (must follow channel guides):
...
6. Create Content Library draft pages:
